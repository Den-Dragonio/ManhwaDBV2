...
export async function renderAccountPage(userId, isOwn = false) {
    ...
    <div class="page-container">
      ...
      <div class="profile-header">
        ...
        <div class="profile-info">
          ...
          ${isOwn ? `<div style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn-secondary btn-sm" id="edit-account-btn">⚙️ Налаштування</button><button class="btn btn-secondary btn-sm" id="stats-btn" style="${reviews.filter(r => r.type === 'manhwa').length < 10 ? 'opacity:0.6;cursor:default' : ''}">📊 Детальна інформація</button></div>` : ''}
  ...
  if (isOwn) {
    ...
    document.getElementById('stats-btn')?.addEventListener('click', () => {
      const manhwaCount = reviews.filter(r => r.type === 'manhwa').length;
      if (manhwaCount < 10) {
        showToast(`📊 Детальна статистика доступна після 10 рецензій (у вас ${manhwaCount})`, 'info');
        return;
      }
      navigate('stats');
    });
